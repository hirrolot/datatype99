...
| `DATATYPE99_attrValue`/`DATATYPE99_ATTR_VALUE` | Accepts an attribute name and expands to its value. A provided attribute **must** be present. |
| `DATATYPE99_assertAttrIsPresent` | Accepts an attribute name and emits a fatal error if the attribute is not present, otherwise results in emptiness. |
